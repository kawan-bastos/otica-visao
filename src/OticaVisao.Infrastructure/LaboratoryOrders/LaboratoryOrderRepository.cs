...
public sealed class LaboratoryOrderRepository(ApplicationDbContext context) : ILaboratoryOrderRepository
{
    public async Task<IReadOnlyList<LaboratoryOrder>> ListAsync(CancellationToken cancellationToken = default) =>
        await Query().AsNoTracking().OrderBy(order => order.Status == LaboratoryOrderStatus.Delivered || order.Status == LaboratoryOrderStatus.Cancelled)
    ...
    public async Task<IReadOnlyList<LaboratoryOrder>> ListBySaleIdAsync(Guid saleId, CancellationToken cancellationToken = default) =>
        await Query().Where(order => order.SaleId == saleId).ToListAsync(cancellationToken);
    ...
    public void RemoveRange(IEnumerable<LaboratoryOrder> orders) => context.LaboratoryOrders.RemoveRange(orders);
